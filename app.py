...
# Versión 4.2 FINAL – Corregida para Streamlit Cloud
# ==============================================================
...
# 🧠 Sistema Inteligente de Modelado del Precio de la Soya – SolverTic SRL
# Versión 4.4 FINAL (100% libre de “Oh no” en Streamlit Cloud)
# ==============================================================
        ...
        try:
            if include_fourier:
                for K in range(K_min, K_max+1):
                    ...
                    results.append({'order': order, 'seasonal': seasonal_order,
                                    'fourier_K': K, 'aic': res.aic,
                                    'mape': mape(test, fc),
...
def buscar_modelos(train, test, pmax, qmax, Pmax, Qmax, periodo, include_fourier, K_min, K_max):
    ...
    for i, (p, q, P, Q) in enumerate([(p, q, P, Q)
                                      for p in range(pmax+1)
                                      for q in range(qmax+1)
                                      for P in range(Pmax+1)
                                      for Q in range(Qmax+1)]):
        ...
        try:
            if include_fourier:
                for K in range(K_min, K_max+1):
                    ...
                    jb_p, lb_p, arch_p, resid = diagnosticos(res)
                    results.append({
                        'order': order, 'seasonal': seasonal_order, 'fourier_K': K,
